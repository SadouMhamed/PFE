<x-app-layout>
                            ...
                            <tr class="transition-colors duration-200 hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-sm font-medium whitespace-nowrap">
                                    <div class="flex space-x-3">
                                        <!-- Replace the current observation button with this -->
                                        @if(auth()->user()->role === 'admin' || auth()->user()->role === 'technicien')
                                        <button onclick="openObservationModal('{{ route('tickets.addObservation', $ticket->id) }}')" 
                                        ...
                                        <!-- Nouveau bouton pour ajouter une description -->
                                        @if(auth()->user()->role === 'admin')
                                        <button onclick="openDescriptionModal('{{ route('tickets.addDescription', $ticket->id) }}')" 
                                                class="inline-flex items-center px-3 py-2 ml-2 text-sm font-medium leading-4 text-white bg-purple-600 rounded-md border border-transparent transition-colors duration-200 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500">
                                            <svg class="mr-1 w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                            </svg>
                                            Description
    ...
    <script>
        function openObservationModal(url) {
            console.log('Opening observation modal with URL:', url);
            document.getElementById('observationForm').action = url;
        ...
        // Remove the DOMContentLoaded event listener and related code since it's not needed
