        ...
        <div class="mx-auto max-w-3xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h2 class="mb-4 text-lg font-medium text-gray-900">Edit Admin User</h2>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="wilaya_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('wilaya_id') border-red-500 @enderror" 
                                ...
                                @foreach($wilayas as $wilaya)
                                    <option value="{{ $wilaya->id }}" {{ old('wilaya_id', $user->wilaya_id) == $wilaya->id ? 'selected' : '' }}>{{ $wilaya->wilaya_name }}</option>
                                @endforeach
                            </select>
                            @error('wilaya_id')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
